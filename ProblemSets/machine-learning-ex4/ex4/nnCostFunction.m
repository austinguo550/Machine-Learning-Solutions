...
regularizeTheta1 = Theta1(:,2:size(Theta1,2));
regularizeTheta2 = Theta2(:,2:size(Theta2,2));
             
J = 1/m * sum(sum(-y .* log(h) - (1-y) .* log(1-h), 2)) + lambda/(2*m) * (sum(sum(regularizeTheta1.^2,2)) + sum(sum(regularizeTheta2.^2,2)));
